...
public class PokedexClient {
	...
	public PokemonBasicResponse getPokemonBasicInformation(String name) throws Exception {
		...
		Mono<String> responseFromApi = webClient.get().uri("https://pokeapi.co/api/v2/pokemon/" + name)
			.retrieve()
	...
	public PokemonEvolutionsResponse getPokemonEvolutions(String id) {
		PokemonEvolutionsResponse response = new PokemonEvolutionsResponse();
		Mono<String> responseFromApi = webClient.get().uri("https://pokeapi.co/api/v2/evolution-chain/" + id)
			.retrieve()
			.bodyToMono(String.class);
		JsonObject jsonObject = new JsonParser().parse(responseFromApi.block()).getAsJsonObject();
		JsonElement jsonWeight = jsonObject.get("chain");
		response.setChain(jsonWeight.getAsJsonObject());
		return response;
	}
